...
module('Integration | Component | situation-form/radios', function(hooks) {
  ...
  test('it renders', async function(assert) {
    await render(
      hbs`<SituationForm::Radios @legend={{this.legend}} @name={{this.name}} @options={{this.options}} @chosen=random @action={{this.click}} />`
    );

    assert
      .dom('[data-test-parameters-radio-fieldset=difficulty-mode]')
      .hasAttribute('id', 'difficulty-mode');
    assert
      .dom('[data-test-parameters-radio-legend=difficulty-mode]')
      .hasText('Mode');
    this.options.forEach(difficultyMode => {
      let radioSelector = `[data-test-parameters-radio='difficulty-mode:${difficultyMode}']`;
      assert
        .dom(radioSelector)
        .hasAttribute('id', `difficulty-mode:${difficultyMode}`);
      ...
      assert
        .dom(labelSelector)
        .hasAttribute('for', `difficulty-mode:${difficultyMode}`);
    });
    assert
      .dom('[data-test-parameters-radio="difficulty-mode:random"]')
      .isChecked();
    ['standard', 'expert'].forEach(difficultyMode => {
      assert
        .dom(`[data-test-parameters-radio='difficulty-mode:${difficultyMode}']`)
        .isNotChecked();
    });
  });
  ...
  test('it renders the chosen option', async function(assert) {
    await render(
      hbs`<SituationForm::Radios @legend={{this.legend}} @name={{this.name}} @options={{this.options}} @chosen=standard @action={{this.click}} />`
    );

    assert
      .dom('[data-test-parameters-radio="difficulty-mode:standard"]')
      .isChecked();
